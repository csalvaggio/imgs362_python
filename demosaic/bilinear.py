...
if __name__ == '__main__':
   ...
   rgb = cv2.merge((b,g,r))
   filename = os.path.join(path, 'image_bilinear.ppm')
   cv2.imwrite(filename, rgb * scale)

   rgb = ipcv.gamma_correction(rgb, gamma=gamma, maxCount=maxCount)
   filename = os.path.join(path, 'image_bilinear_gamma.ppm')
   cv2.imwrite(filename, rgb * scale)

   rgb = ipcv.awb.grayworld(rgb, scalingFactor=1.1, maxCount=maxCount)
   filename = os.path.join(path, 'image_bilinear_gamma_awb.ppm')
   cv2.imwrite(filename, rgb * scale)
