...
def laroche_and_prescott(cfa, pattern='GBRG', maxCount=65535):
   ...
   numberRows, numberColumns, numberBands, dataType = ipcv.dimensions(cfa)
...
if __name__ == '__main__':
   ...
   path = os.path.join(home, 'src', 'python', 'examples', 'data', 'raw', 'rpi')
   filename = os.path.join(path, 'image.pgm')
   pattern = 'GBRG'
   maxCount = 65535
   gamma = 2.2
   scale = 1
   ...
   rgb = cv2.merge((b,g,r))
   filename = os.path.join(path, 'image_laroche_and_prescott.ppm')      
   cv2.imwrite(filename, rgb * scale)

   rgb = ipcv.gamma_correction(rgb, gamma=gamma, maxCount=maxCount)
   filename = os.path.join(path, 'image_laroche_and_prescott_gamma.ppm')
   cv2.imwrite(filename, rgb * scale)

   rgb = ipcv.awb.grayworld(rgb, scalingFactor=1.1, maxCount=maxCount)
   filename = os.path.join(path, 'image_laroche_and_prescott_gamma_awb.ppm')
   cv2.imwrite(filename, rgb * scale)
